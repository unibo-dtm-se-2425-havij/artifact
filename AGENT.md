...
## Git workflow
- Default branch is `main`. All changes branch from `main`.
- Branching style follows git-flow conventions (feature/ release/ hotfix branches off `main` as appropriate).
- Commit messages follow Conventional Commits.
- Each commit in `main` creates a release automatically that bumps the version itself. So no version bump is needed.
