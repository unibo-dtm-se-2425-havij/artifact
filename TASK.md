...
- All your project code into a single main package (`havij/`)
- All your project tests into a single test package (`tests/`)
...
```bash
<root directory>
├── havij/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `havij`
...
2. Clone the `havij` repository

3. Open a shell into your local `havij` directory and run
    ```bash
    ./rename-template.sh havij
    ```
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `havij`, in this example)
...
13. Put your main (resp. test) code in `havij/` (resp. `test/`)
...
This will execute the `__main__.py` file in the `havij` package:
```bash
python -m havij
```
